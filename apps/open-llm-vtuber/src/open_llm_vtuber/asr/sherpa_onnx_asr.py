...
class VoiceRecognition(ASRInterface):
    ...
    def _create_recognizer(self):
        if self.model_type == "transducer":
            ...
            if self.hotwords_file and os.path.exists(self.hotwords_file):
                try:
                    ...
                    feat_cfg = FeatureExtractorConfig(sampling_rate=self.SAMPLE_RATE, feature_dim=self.feature_dim)
                    ...
                    recognizer_cfg = OfflineRecognizerConfig(
                        feat_config=feat_cfg,
                        model_config=model_cfg,
                        hotwords_file=self.hotwords_file,
                        hotwords_score=self.hotwords_score,
                    )
                    recognizer = _Recognizer(recognizer_cfg)
                    logger.info(f"Sherpa-Onnx-ASR: Loaded hotwords file '{self.hotwords_file}' (score={self.hotwords_score})")
                except Exception as e:
                    logger.warning(f"Failed to load hotwords in SenseVoice config: {e}")
