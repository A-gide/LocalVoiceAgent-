...
class WebSocketHandler:
    ...
    def __init__(self, default_context_cache: ServiceContext):
        ...
        self.last_interrupt_mono: Dict[str, float] = {}
        self.early_transcription_tasks: Dict[str, Tuple[Optional[asyncio.Task], int]] = {}
    ...
    async def _handle_interrupt(
        ...
        """Handle conversation interruption with 200ms debounce (First-Wins)"""
        now = time.monotonic()
        last_time = self.last_interrupt_mono.get(client_uid, 0.0)
        if now - last_time < 0.200:
            logger.debug(f"Interrupt for client {client_uid} debounced (<200ms window, first-wins)")
            return
        self.last_interrupt_mono[client_uid] = now
    ...
    async def _handle_audio_data(
        ...
        """Handle incoming audio data with A5 Early Transcription on silence gap"""
        audio_data = data.get("audio", [])
        if audio_data:
            chunk_arr = np.array(audio_data, dtype=np.float32)
            self.received_data_buffers[client_uid] = np.append(
                self.received_data_buffers[client_uid],
                chunk_arr,
            )
            # A5: Check for silence gap (energy dip) in ongoing utterance to trigger speculative early ASR
            buf = self.received_data_buffers[client_uid]
            if len(buf) >= 12800:  # >= 0.8s speech accumulated
                max_amp = float(np.max(np.abs(chunk_arr))) if len(chunk_arr) > 0 else 0.0
                existing = self.early_transcription_tasks.get(client_uid)
                if max_amp < 0.03 and (not existing or existing[0].done()):
                    context = self.client_contexts.get(client_uid)
                    if context and hasattr(context, "asr_engine") and context.asr_engine:
                        snapshot = buf.copy()
                        task = asyncio.create_task(context.asr_engine.async_transcribe_np(snapshot))
                        self.early_transcription_tasks[client_uid] = (task, len(snapshot))
                        logger.info(f"⚡ A5: Triggered early transcription on chunk silence gap ({len(snapshot)/16000:.2f}s)")

    async def _handle_raw_audio_data(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Handle incoming raw audio data for VAD processing with A6 Speech-Start instant interrupt and A5 early ASR"""
        context = self.client_contexts[client_uid]
        chunk = data.get("audio", [])
        if chunk:
            for audio_bytes in context.vad_engine.detect_speech(chunk):
                if audio_bytes == b"<|PAUSE|>":
                    # Instant server-side interrupt if active conversation exists
                    active_task = self.current_conversation_tasks.get(client_uid)
                    if active_task and not active_task.done():
                        await self._handle_interrupt(websocket, client_uid, {"text": "vad-pause-interrupt"})
                    await websocket.send_text(
                        json.dumps({"type": "control", "text": "interrupt"})
                    )
                    # A5: Speculative early transcription on VAD pause
                    buf = self.received_data_buffers.get(client_uid)
                    if buf is not None and len(buf) >= 8000:  # >= 0.5s voice
                        existing = self.early_transcription_tasks.get(client_uid)
                        if not existing or existing[0].done():
                            if hasattr(context, "asr_engine") and context.asr_engine:
                                snapshot = buf.copy()
                                task = asyncio.create_task(context.asr_engine.async_transcribe_np(snapshot))
                                self.early_transcription_tasks[client_uid] = (task, len(snapshot))
                                logger.info(f"⚡ A5: Triggered early transcription on VAD pause ({len(snapshot)/16000:.2f}s)")
                elif audio_bytes == b"<|RESUME|>":
                    pass
                elif len(audio_bytes) > 1024:
                    # A6: Speech-start immediate interruption
                    active_task = self.current_conversation_tasks.get(client_uid)
                    if active_task and not active_task.done():
                        await self._handle_interrupt(websocket, client_uid, {"text": "speech-start-interrupt"})
                        await websocket.send_text(
                            json.dumps({"type": "control", "text": "interrupt"})
                        )
    ...
    async def _handle_conversation_trigger(
        ...
        """Handle triggers that start a conversation, passing A5 early transcription task if present"""
        early_info = self.early_transcription_tasks.pop(client_uid, (None, 0))
        if early_info[0]:
            if "metadata" not in data or not isinstance(data.get("metadata"), dict):
                data["metadata"] = {}
            data["metadata"]["early_asr_task"] = early_info[0]
            data["metadata"]["early_asr_samples"] = early_info[1]
            logger.info(f"⚡ A5: Passing early transcription task ({early_info[1]} samples) into conversation pipeline")
