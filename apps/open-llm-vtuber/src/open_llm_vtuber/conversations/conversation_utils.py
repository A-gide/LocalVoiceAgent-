...
async def process_agent_output(
    ...
    """Process agent output with character information, optional translation, and turn isolation"""
...
async def handle_sentence_output(
    ...
    """Handle sentence output type with optional translation support and turn isolation"""
    full_response = ""
...
async def process_user_input(
    ...
    websocket_send: WebSocketSend,
    early_asr_task: Optional[asyncio.Task] = None,
    early_asr_samples: int = 0,
) -> str:
    """Process user input, converting audio to text if needed, reusing early transcription if available (A5)"""
    if isinstance(user_input, np.ndarray):
        if early_asr_task:
            try:
                early_text = await early_asr_task
                delta_samples = len(user_input) - early_asr_samples
                # If trailing difference is minimal (e.g. <= 400ms trailing silence)
                if delta_samples <= 16000 * 0.4:
                    input_text = early_text
                    logger.info(f"⚡ A5 Early Transcription: Reused pre-transcription with 0ms wait: '{input_text}'")
                elif delta_samples > 0 and len(user_input) > early_asr_samples:
                    tail_text = await asr_engine.async_transcribe_np(user_input[early_asr_samples:])
                    input_text = f"{early_text} {tail_text}".strip()
                    logger.info(f"⚡ A5 Early Transcription: Stitched pre-transcription + tail: '{input_text}'")
                else:
                    input_text = early_text
            except Exception as e:
                logger.warning(f"Early transcription task failed, falling back to full transcribe: {e}")
